...
function Notificationandidate({ candidate_id }) {
  const [notifications, setNotifications] = useState(null);
  const [buttonType, setButtonType] = useState({
    notification_id: null,
    type: null,
  });
  const fetchCandidateNotifications = () => {
    const today_date = new Date().toISOString();
    const payload = {
      endpoint: `get-candidate-notification?candidate_id=${candidate_id}&&today_date=${today_date}`,
      method: "GET",
    };
...
export default Notificationandidate;
