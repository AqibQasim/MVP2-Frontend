 ...
 const fetchCustomers = async (startingAfter = null) => {
  try {
    ...
    if (response.ok) {
      ...
      setLastCustomerId(data.data.length > 0 ? data.data[data.data.length - 1].id : null);
    } else {
      setError(data.error);
    }
  } catch (error) {
      ...
      <div className='flex justify-between'>
        ...
        <div>
          ...
          <h1 className='text-lg'>
            Available: <span className='font-semibold'>{balance.available.map((avail) => `${(avail.amount / 100).toFixed(2)} $`).join(', ')}</span>
          </h1>
          <h1 className='text-lg'>
            Pending: <span className='font-semibold'>{balance.pending.map((pend) => `${(pend.amount / 100).toFixed(2)} $`).join(', ')}</span>
          </h1>
        </div>
      </div>
      ...
      <div className='h-[90vh] p-5 overflow-y-auto'>
        <ul className='flex flex-col gap-3'>
          ...
          {filteredCustomers.map((customer) => (
            <>
            <li className='grid grid-cols-5 text-start' key={customer.id}>
              <div>{customer.id}</div>
              <div>{customer.name}</div>
              <div>{customer.email}</div>
               <button onClick={() => openInvoiceModal(customer)} className="btn-primary">
              Create Invoice
            </button>
              <Capsule
                onClick={() => handleCustomerClick(customer.id)}
                className="mx-auto w-max !bg-primary-tint-100 cursor-pointer"
              >
                View History
              </Capsule>
